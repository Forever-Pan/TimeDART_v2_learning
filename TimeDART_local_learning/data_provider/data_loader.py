...
# UEA datasets
# 继承自 PyTorch 的 Dataset 类，用于加载和处理癫痫相关的数据集
# 其主要功能为从指定的路径加载数据和标签，提供数据的基本属性（如样本数量和序列长度），实现 __getitem__ 和 __len__ 方法，使其可以被 PyTorch 的 DataLoader 使用，用于按索引访问数据和获取数据集长度
class Dataset_Epilepsy(Dataset):
    # 初始化方法，接受数据集的根路径、文件列表、限制大小和标志等参数
    def __init__(self, root_path, file_list=None, limit_size=None, flag=None, **kwargs):
        # 将参数赋值给实例变量（self）
        self.kwargs = kwargs
        self.root_path = root_path
        # 调用 load_all 方法加载特征和标签数据
    ...
    def load_all(self, root_path, file_list=None, flag=None):
        ...
        # 根据flag的值选择数据和标签的路径（train、val或test）
        ...
        if flag == 'train':
            ...
            data_p = os.path.join(root_path, 'test_d.npy')
            label_p = os.path.join(root_path, 'test_l.npy')
        else: # 如果flag的值不在'train'、'val'或'test'中，则抛出异常
        ...
        self.length = datas.shape[0] # 样本数量
        self.max_seq_len = datas.shape[1] # 序列长度
        self.class_names = np.unique(labels) # 标签类别
    ...
    # 实现按索引访问数据的逻辑，使数据集可以像列表一样被索引
    def __getitem__(self, ind):
        # data_x:当前索引对应的特征数据，转换为 PyTorch 张量
        # data_y:当前索引对应的标签数据，转换为 PyTorch 张量(long类型)
        data_x = torch.from_numpy(self.feature_df[ind])
        data_y = torch.tensor(self.labels_df[ind], dtype=torch.long)
        # x_mark和y_mark:用于标记特征和标签的占位张量（全零张量），形状与data_x和data_y相同
