...
# 用于时间序列数据处理的模块，负责将输入序列划分为固定长度的片段（patches）
class Patch(nn.Module):
    def __init__(
        self,
        patch_len: int, # 每个 patch 的长度，表示每个片段包含的时间步数
        stride: int, # 滑动窗口的步幅，决定了相邻 patch 之间的重叠程度
    ):
        super(Patch, self).__init__()
        # 将 patch_len 和 stride 存储为类的属性，供后续的 forward 方法使用
        self.patch_len = patch_len
        self.stride = stride

    # 前向传播方法：
    # 输入张量 x 的形状为 [batch_size * num_features, input_len, 1]，其中：batch_size 是批量大小。num_features 是特征数量。input_len 是时间序列的长度。
    # 数据处理：使用 squeeze(-1) 去掉最后一维，将输入张量的形状变为 [batch_size * num_features, input_len]。调用 unfold 方法对时间序列进行滑动窗口操作：patch_len 决定每个窗口的长度。stride 决定窗口的滑动步幅。unfold 的结果是一个新的张量，形状为 [batch_size * num_features, num_patches, patch_len]，其中：num_patches = (input_len - patch_len) // stride + 1，表示划分出的 patch 数量。
    # 输出数据：返回划分后的张量，形状为 [batch_size * num_features, num_patches, patch_len]，每个 patch 包含 patch_len 个时间步。
    def forward(self, x):
        """
...
# 用于时间序列数据处理的模块，负责将划分后的时间序列片段（patches）映射到高维特征空间。它通过一个全连接层（nn.Linear）实现这种映射操作
...
# 用于时间序列或序列建模的模块，负责为输入数据添加位置信息（Positional Encoding）。在深度学习模型（如 Transformer）中，位置信息对于捕获序列的时间依赖性至关重要，因为模型本身对输入序列的顺序没有内置的感知能力
