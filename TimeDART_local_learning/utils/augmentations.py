# 一系列用于时间序列数据增强和掩码生成的函数，主要用于数据预处理和模型训练中的数据增强

import numpy as np
import torch
import math

# 对时间序列数据进行掩码操作，生成掩码后的数据
def masked_data(sample, sample_mark, masking_ratio, lm, positive_nums=1, distribution='geometric'):
    ...
    # 将输入数据 sample 的维度从 [batch_size, seq_len, nvars] 转换为 [batch_size, nvars, seq_len]
...
# 为输入数据 X 生成一个布尔掩码矩阵，用于掩码特定位置的特征
...
def one_hot_encoding(X):
    ...
    X = [int(x) for x in X]
    n_values = np.max(X) + 1
    b = np.eye(n_values)[X] # 使用 NumPy 的 eye 函数生成独热编码矩阵
    return b

# 对输入数据进行弱增强和强增强
def DataTransform(sample, config):
    """Weak and strong augmentations"""
    # 弱增强通过 scaling 函数实现（如缩放操作）
    weak_aug = scaling(sample, config.augmentation.jitter_scale_ratio)
    # weak_aug = permutation(sample, max_segments=config.augmentation.max_seg)
    # 强增强通过 jitter 和 permutation 函数实现（如抖动和随机排列）
