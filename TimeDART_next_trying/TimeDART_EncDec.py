...
# 改动后将ARFlattenHead的线性层改为两层线性层
# 参数量：d_model ^ 2//2 + d_model * patch_size // 2 = 896 * 448 + 448 * 2 = 402304
class ARFlattenHead(nn.Module):
    def __init__(
        self,
        d_model: int,
        ...
        super(ARFlattenHead, self).__init__()
        self.flatten = nn.Flatten(start_dim=-2)
        self.fc1 = nn.Linear(d_model, d_model // 2)  # 第一层线性层
        self.relu = nn.ReLU()  # 激活函数
        self.fc2 = nn.Linear(d_model // 2, patch_len)  # 第二层线性层
        self.dropout = nn.Dropout(dropout)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        :return: [batch_size, seq_len * patch_len, num_features]
        """
        x = self.fc1(x)  # (batch_size, num_features, seq_len, d_model // 2)
        x = self.relu(x)  # 激活函数
        x = self.fc2(x)  # (batch_size, num_features, seq_len, patch_len)
