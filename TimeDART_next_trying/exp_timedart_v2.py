...
class Exp_TimeDART_v2(Exp_Basic):
    ...
    def _build_model(self):
        ...
        # # ptraining + finetuning 时不注释，单独finetuning 时注释
        ...
        #     # )
        #     checkpoint = torch.load(self.args.load_checkpoints, map_location=lambda storage, loc: storage.cuda(0))
    ...
    def pretrain(self):
        ...
        # ExponentialLR 学习率调度器：每个 epoch 后，学习率按照 gamma=self.args.lr_decay 的比例进行指数衰减。学习率逐渐减小，衰减速度由 lr_decay 控制
        # model_scheduler = torch.optim.lr_scheduler.ExponentialLR(
        #     optimizer=model_optim, gamma=self.args.lr_decay
        # )

        # 将 ExponentialLR 替换为 CosineAnnealingLR，并设置 T_max 为总 step 数
        # 基于 step 的 cosine 学习率调整：学习率在每个 step（batch）后按照余弦函数衰减。适用于长时间训练任务，学习率在每个 step 内动态变化。
        total_steps = len(train_loader) * self.args.train_epochs
        model_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            optimizer=model_optim, T_max=total_steps, eta_min=0
        )
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(
                ...
                train_loss.append(diff_loss.item() * accumulation_steps)
    ...
    def train(self, setting):
        ...
        # OneCycleLR 学习率调度器：学习率在一个周期内先增大到 max_lr，然后再逐渐减小。适用于单周期训练任务，学习率在训练中动态变化。
        ...
        # 将 OneCycleLR 替换为 CosineAnnealingLR，并设置 T_max 为总 step 数。
        # 基于 step 的 cosine 学习率调整：学习率在每个 step（batch）后按照余弦函数衰减。适用于长时间训练任务，学习率在每个 step 内动态变化。
        total_steps = len(train_loader) * self.args.train_epochs
        model_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            optimizer=model_optim, T_max=total_steps, eta_min=0
