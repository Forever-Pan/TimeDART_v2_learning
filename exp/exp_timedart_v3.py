...
class Exp_TimeDART_v3(Exp_Basic):
    def __init__(self, args):
        super(Exp_TimeDART_v3, self).__init__(args)
    ...
    def _build_model(self):
        ...
        # # 手动迁移部分预训练权重
        # if self.args.load_checkpoints:
        #     print(f"Loading pretrained weights from: {self.args.load_checkpoints}")
        #     pretrained_model = torch.load(self.args.load_checkpoints, map_location=self.device)
        #     self.map_pretrained_weights(pretrained_model, model)
        ...
        if self.args.load_checkpoints:
            ...
            temp_model.load_state_dict(pretrained_model, strict=False)
            
            # 使用 map_pretrained_weights 方法迁移权重
            self.map_pretrained_weights(temp_model, model)
