...
from transformers import AutoModelForCausalLM # 导入AutoModelForCausalLM
...
class Model(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.args = args # 不知道为什么原先的TimeDART没有这部分初始化
    ...
    # 将 TimeDART_v2 中的 __init_encoder 方法迁移到 TimeDART 中
    ...
    def pretrain(self, x):
        ...
        # x_out = self.encoder(
        #     inputs_embeds=x_embedding_bias, # 需要明确指定 inputs_embeds 参数
        #     is_mask=True,
        # )  # [batch_size * num_features, seq_len, d_model]

        # # 提取最后一层 hidden_states 作为输出
        # x_out = x_out.hidden_states[-1]  # [batch_size * num_features, seq_len, d_model]

        # Qwen2.5-0.5B 的 encoder 需要指定 inputs_embeds 参数
